/* =====================================================================
   ...
   ===================================================================== */
const CONFIG = {
  telegramUsername: 'nokiamane', // без @, используется для чекаута
  ...
  aboutParagraphs: [
    `SPOTTER LIVE — русскоязычная площадка грайма: артисты читают вживую под бит,
    в один заход, без десятков дублей. Идею основатель REDO вынашивал ещё с 2018 года —
    тогда проект должен был называться GREEN STREET и выходить в Петербурге, но не сложился.`,

    `В 2023-м REDO полгода пытался отдать концепцию организаторам — все отказывались,
    один из лейблов прямо сказал, что такой грайм слушать никто не будет. Тогда он
    запустил проект сам и пригласил режиссёра Матвея Николаева — того самого, чьи клипы
    выходят на канале сейчас — чтобы сделать визуальный стиль SPOTTER узнаваемым.`,

    `Через год после запуска проект вышел на международную аудиторию: около 30% зрителей —
    из Европы и Великобритании, 70% — русскоязычные, которые с первого дня рядом. По словам
    REDO, проект поддержал Wiley — один из пионеров жанра в Великобритании.`
  ],
