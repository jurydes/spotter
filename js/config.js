...
const CONFIG = {
  ...
  // Показывать ли опрос посетителям.
  //
  // false — с карточек пропадает кнопка «Пройти опрос» и блок со скидкой.
  // Сам опрос при этом цел: вопросы, экраны, проверка ответов, выгрузка
  // в Google-таблицу и скидка лежат в коде нетронутыми. Захотите вернуть —
  // поставьте true, больше ничего делать не нужно.
  surveyEnabled: false,
  ...
  // Учитывать ли уже оформленные заказы в остатках.
  //
  // false (сейчас) — остатки на сайте не уменьшаются, заказать можно
  // сколько угодно и сколько угодно раз. Это осознанно: идёт предзаказ,
  // тираж ещё не отшит, и упереться в цифру раньше времени хуже, чем
  // разобрать перебор вручную.
  //
  // true — сайт спрашивает у таблицы, сколько уже заказано, вычитает это
  // из тиража в админке и не даёт заказать сверх него; скрипт проверяет
  // то же самое ещё раз в момент заказа. Лист «Резерв» в таблице ведётся
  // в любом случае, так что включение ничего не потеряет — см.
  // serverless/google-sheets/README.md.
  reserveStock: false,
  ...
  // Номер счётчика Яндекс Метрики. Пустая строка — аналитики нет совсем:
  // счётчик не грузится, куки не ставятся. Взять номер: metrika.yandex.ru →
  // «Добавить счётчик». Цели заводятся там же, их имена перечислены
  // в js/analytics.js.
  metrikaId: '112876563',
  ...
  aboutParagraphs: [
    ...
    `В 2025-м REDO полгода пытался предложить концепцию организаторам, но получал
    ...
    он пригласил режиссёра Матвея Николаева — чтобы с самого начала сформировать
    узнаваемый визуальный язык SPOTTER.`,

    `За первый год SPOTTER вышел за пределы русскоязычной сцены. Сегодня около 20%
    аудитории проекта приходится на Европу и Великобританию, остальные 80% — на
    русскоязычных зрителей, которые следят за SPOTTER с первых дней. Проект заметили
    и за пределами России: его поддержал Wiley — один из пионеров британского грайма.`
  ],
...
/* =====================================================================
   ...
   ВАЖНО: это запасной снимок, как и с выпусками выше. Настоящий источник —
   data/artists.json, раздел «Участники» в редакторе. Руками здесь обычно
   ничего делать не нужно: правка из редактора перекрывает эту таблицу.
   ===================================================================== */
const ARTIST_BIOS = {
